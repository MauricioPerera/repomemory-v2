...
- **Prompt templates (v2.16.0)**: `src/recall/templates.ts` defines configurable templates controlling section order, headers, preamble, and per-collection weight multipliers. 6 built-in templates: `default`, `technical`, `support`, `rag_focused`, `few_shot`, `a2e`. Templates resolved via `resolveTemplate()`. RecallEngine applies collection weight multipliers before score pooling. Do NOT change the weight normalization in `computeScore()` — template weights are applied externally as score multipliers.
...
- **A2E Integration (v2.18.0)**: `src/a2e/` module integrates the A2E (Agent-to-Execution) protocol. Three sub-modules: `sanitize.ts` (secret resolution/sanitization), `circuit-breaker.ts` (error-based host blocking), `workflow-skills.ts` (save/recall/mine workflow patterns). Three MCP tools: `a2e_save_workflow`, `a2e_recall_workflows`, `a2e_check_circuit`. RecallEngine's few-shot extraction recognizes `[A2E: ...]` tags. Workflow skills use `saveOrUpdate()` with category `fact`; errors use category `correction` with `a2e-error` tag for circuit breaker detection. See `docs/A2E_INTEGRATION.md`.
- **A2E Validation Pipeline (v2.19.0)**: `src/a2e/validate.ts` provides a three-stage pipeline: `normalizeResponse()` (strips `<think>` tags, collapses pretty-print JSON, extracts from code blocks, reorders beginExecution to end) → `fixJsonl()` (fixes unquoted keys/values, trailing commas, repairs truncated JSON via brace counting) → `validateWorkflow()` (validates structure, auto-synthesizes missing beginExecution from defined operationIds). Returns `autoFixed: true` when beginExecution was synthesized. Tested across 11 models (1B to 120B) — enables 1B models to produce valid workflows on first attempt. The `a2e` template uses skills-first ordering with 2.0x weight, 7 few-shot examples, and direct preamble. One template serves all model sizes. See `docs/CTT_RESEARCH_REPORT.md`.
- **MCP tool count (v2.18.0)**: 38 tools total (added 3 A2E tools). New tool: `export_filtered`. The `recall` tool now returns `fewShotExamples` when using templates with `extractFewShot: true`.
- **Few-shot extraction (v2.17.0)**: Skills containing tool-calling patterns (`[MCP: ...]`, `[CALC: ...]`, `[FETCH: ...]`) are converted to user/assistant conversation pairs by `RecallEngine`. Designed for sub-1B models that learn tool usage better from demonstrated examples. Activated via `few_shot` template or custom templates with `extractFewShot: true`. Developed in MicroExpert, backported here.
- **Memory Packs / Filtered export (v2.17.0)**: `exportFiltered()` supports query, tags, types, agentId, and userId filtering. Returns v2 pack format (`version: 2`) when `PackMetadata` is provided — distributable versioned JSON files solving the cold-start problem for new agent instances. MCP tool: `export_filtered`.
