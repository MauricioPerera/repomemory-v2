...
- **Prompt templates (v2.16.0)**: `src/recall/templates.ts` defines configurable templates controlling section order, headers, preamble, and per-collection weight multipliers. 5 built-in templates: `default`, `technical`, `support`, `rag_focused`, `few_shot`. Templates resolved via `resolveTemplate()`. RecallEngine applies collection weight multipliers before score pooling. Do NOT change the weight normalization in `computeScore()` — template weights are applied externally as score multipliers.
...
- **MCP tool count (v2.17.0)**: 35 tools total. New tool: `export_filtered`. The `recall` tool now returns `fewShotExamples` when using templates with `extractFewShot: true`.
