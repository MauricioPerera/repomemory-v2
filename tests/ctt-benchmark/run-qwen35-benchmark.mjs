#!/usr/bin/env node
/**
 * CTT Benchmark for Qwen3.5:0.8b — Think vs No-Think comparison.
 * Runs the full benchmark twice: once with reasoning enabled, once without.
 * Uses Ollama native API (/api/chat) for think parameter control.
 *
 * Usage: OLLAMA_BASE_URL=http://localhost:11434 node run-qwen35-benchmark.mjs
 */
...
async function ollamaChat(model, messages, think = true, timeoutMs = 120_000) {
  ...
  try {
    ...
    // Return both thinking content and response content
    return {
      content: data.message?.content ?? '',
      thinking: data.message?.thinking_content ?? '',
      totalDuration: data.total_duration ? data.total_duration / 1e6 : 0, // ns → ms
...
async function runPass(model, thinkMode, label) {
  console.log(`\n${'═'.repeat(60)}`);
  console.log(`  ${label} (think=${thinkMode})`);
  ...
  for (const domain of domains) {
    ...
    try {
      ...
      for (const tq of domain.testQueries) {
        ...
        let baseResp;
        try {
          baseResp = await ollamaChat(model, [{ role: 'user', content: tq.query }], thinkMode, 180_000);
        } catch (e) {
          console.error(` ✗ failed: ${e.message}`);
          baseResp = { content: '', thinking: '', totalDuration: 0, evalCount: 0 };
        }
        ...
        try {
          ...
          console.error(` ✗ failed: ${e.message}`);
          cttResp = { content: '', thinking: '', totalDuration: 0, evalCount: 0 };
        }
        ...
        queryDetails.push({
          query: tq.query,
          base: `${bs.topicHits}/${tq.expectedTopics.length}t ${bs.factHits}/${tq.expectedFacts.length}f (${(baseMs/1000).toFixed(1)}s, ${baseResp.evalCount}tok)`,
          ctt: `${cs.topicHits}/${tq.expectedTopics.length}t ${cs.factHits}/${tq.expectedFacts.length}f (${(cttMs/1000).toFixed(1)}s, ${cttResp.evalCount}tok)`,
          baseResponse: baseResp.content.slice(0, 500),
          cttResponse: cttResp.content.slice(0, 500),
          baseThinking: baseResp.thinking ? baseResp.thinking.slice(0, 200) : '',
          cttThinking: cttResp.thinking ? cttResp.thinking.slice(0, 200) : '',
        });
      }
    } finally {
    ...
    console.log(`  ${domain.name}: Base ${(baseScore*100).toFixed(0)}% → CTT ${(cttScore*100).toFixed(0)}% (${isFinite(improvement) ? `+${improvement.toFixed(0)}%` : 'N/A→CTT'}) | ${(baseLatency/nQ/1000).toFixed(1)}s / ${(cttLatency/nQ/1000).toFixed(1)}s | ${baseTokens}/${cttTokens} tokens`);

    passResults.push({
      model, domain: domain.name, thinkMode,
...
// ─── Main ────────────────────────────────────────────────────────────────────
async function main() {
  console.log(`\n🔬 CTT Benchmark — Qwen3.5 Think vs No-Think`);
  console.log(`   Model: ${MODEL}`);
  console.log(`   Ollama: ${OLLAMA_BASE}\n`);

  // Run WITHOUT thinking
  const noThinkResults = await runPass(MODEL, false, `${MODEL} — NO REASONING (think=false)`);

  // Run WITH thinking
  const thinkResults = await runPass(MODEL, true, `${MODEL} — WITH REASONING (think=true)`);

  // ─── Comparison Report ────────────────────────────────────────────────────
  const timestamp = new Date().toISOString().slice(0, 10);
  const lines = [];
  lines.push(`# CTT Benchmark: ${MODEL} — Think vs No-Think — ${timestamp}`);
  lines.push('');
  lines.push('## Summary');
  lines.push('');
  lines.push('| Domain | Mode | Base Score | CTT Score | Improvement | Avg Base Latency | Avg CTT Latency |');
  lines.push('|--------|------|-----------|-----------|-------------|-----------------|-----------------|');

  for (const r of noThinkResults) {
    const imp = isFinite(r.improvement) ? `+${r.improvement.toFixed(0)}%` : 'N/A→CTT';
    lines.push(`| ${r.domain} | **No-Think** | ${(r.baseScore*100).toFixed(0)}% | ${(r.cttScore*100).toFixed(0)}% | ${imp} | ${(r.avgBaseLatencyMs/1000).toFixed(1)}s | ${(r.avgCttLatencyMs/1000).toFixed(1)}s |`);
  }
  for (const r of thinkResults) {
    const imp = isFinite(r.improvement) ? `+${r.improvement.toFixed(0)}%` : 'N/A→CTT';
    lines.push(`| ${r.domain} | **Think** | ${(r.baseScore*100).toFixed(0)}% | ${(r.cttScore*100).toFixed(0)}% | ${imp} | ${(r.avgBaseLatencyMs/1000).toFixed(1)}s | ${(r.avgCttLatencyMs/1000).toFixed(1)}s |`);
  }

  // Averages comparison
  lines.push('');
  lines.push('## Aggregated Comparison');
  lines.push('');

  const avgNoThink = {
    base: noThinkResults.reduce((s,r) => s + r.baseScore, 0) / noThinkResults.length,
    ctt: noThinkResults.reduce((s,r) => s + r.cttScore, 0) / noThinkResults.length,
    baseLat: noThinkResults.reduce((s,r) => s + r.avgBaseLatencyMs, 0) / noThinkResults.length,
    cttLat: noThinkResults.reduce((s,r) => s + r.avgCttLatencyMs, 0) / noThinkResults.length,
  };
  const avgThink = {
    base: thinkResults.reduce((s,r) => s + r.baseScore, 0) / thinkResults.length,
    ctt: thinkResults.reduce((s,r) => s + r.cttScore, 0) / thinkResults.length,
    baseLat: thinkResults.reduce((s,r) => s + r.avgBaseLatencyMs, 0) / thinkResults.length,
    cttLat: thinkResults.reduce((s,r) => s + r.avgCttLatencyMs, 0) / thinkResults.length,
  };

  lines.push('| Metric | No-Think | Think | Winner |');
  lines.push('|--------|----------|-------|--------|');
  lines.push(`| Avg Base Score | ${(avgNoThink.base*100).toFixed(0)}% | ${(avgThink.base*100).toFixed(0)}% | ${avgNoThink.base > avgThink.base ? 'No-Think' : avgThink.base > avgNoThink.base ? 'Think' : 'Tie'} |`);
  lines.push(`| Avg CTT Score | ${(avgNoThink.ctt*100).toFixed(0)}% | ${(avgThink.ctt*100).toFixed(0)}% | ${avgNoThink.ctt > avgThink.ctt ? 'No-Think' : avgThink.ctt > avgNoThink.ctt ? 'Think' : 'Tie'} |`);
  lines.push(`| Avg Base Latency | ${(avgNoThink.baseLat/1000).toFixed(1)}s | ${(avgThink.baseLat/1000).toFixed(1)}s | ${avgNoThink.baseLat < avgThink.baseLat ? 'No-Think' : 'Think'} |`);
  lines.push(`| Avg CTT Latency | ${(avgNoThink.cttLat/1000).toFixed(1)}s | ${(avgThink.cttLat/1000).toFixed(1)}s | ${avgNoThink.cttLat < avgThink.cttLat ? 'No-Think' : 'Think'} |`);

  const noThinkCttImprove = avgNoThink.base > 0 ? ((avgNoThink.ctt - avgNoThink.base) / avgNoThink.base) * 100 : 0;
  const thinkCttImprove = avgThink.base > 0 ? ((avgThink.ctt - avgThink.base) / avgThink.base) * 100 : 0;
  lines.push(`| CTT Improvement | +${noThinkCttImprove.toFixed(0)}% | +${thinkCttImprove.toFixed(0)}% | ${noThinkCttImprove > thinkCttImprove ? 'No-Think' : 'Think'} |`);
  ...
  for (const r of noThinkResults) {
    ...
    for (const q of r.queryDetails) {
      ...
      if (q.baseResponse) lines.push(`- Base response: ${q.baseResponse.slice(0, 300)}`);
      if (q.cttResponse) lines.push(`- CTT response: ${q.cttResponse.slice(0, 300)}`);
      lines.push('');
    }
  }

  lines.push('### Think Mode');
  lines.push('');
  for (const r of thinkResults) {
    lines.push(`#### ${r.domain}`, '');
    for (const q of r.queryDetails) {
      ...
      if (q.baseResponse) lines.push(`- Base response: ${q.baseResponse.slice(0, 300)}`);
      if (q.cttResponse) lines.push(`- CTT response: ${q.cttResponse.slice(0, 300)}`);
      if (q.baseThinking) lines.push(`- Base thinking: _${q.baseThinking}..._`);
      if (q.cttThinking) lines.push(`- CTT thinking: _${q.cttThinking}..._`);
      lines.push('');
    }
  }

  lines.push('---');
  lines.push(`*Generated by RepoMemory CTT Benchmark Framework — ${MODEL} Think vs No-Think*`);

  const report = lines.join('\n');
  const safeModel = MODEL.replace(/[/:]/g, '-');
  const reportPath = join(process.cwd(), `ctt-benchmark-${safeModel}-think-${timestamp}.md`);
